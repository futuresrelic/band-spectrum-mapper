...
import IconManagerPage from './IconManagerPage';
...
export default function SettingsPage() {
  ...
  return (
    <div>
      <PageHeader title="Settings" subtitle="Configure analysis behaviour and site branding" />

      <div className="flex gap-1 mb-6 border-b border-surface-200">
        {([['stopwords', 'Stopwords'], ['icons', 'App Icons']] as [Tab, string][]).map(([t, label]) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors ${tab === t ? 'border-surface-900 text-surface-900' : 'border-transparent text-surface-500 hover:text-surface-700'}`}
          >
            {label}
          </button>
        ))}
