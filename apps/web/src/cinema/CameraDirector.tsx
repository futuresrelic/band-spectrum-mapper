...
export default function CameraDirector({
  ...
  function updateKf(id: string, patch: Partial<CinemaKeyframe>) {
    setKeyframes(keyframes.map(k => k.id === id ? { ...k, ...patch } : k));
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="text-[10px] text-gray-500 leading-snug px-0.5">
        Navigate freely, then capture. Build a sequence shot by shot.
      </div>
      ...
      {keyframes.length === 0 && (
        <div className="text-center text-[11px] text-gray-700 py-3">
          No shots yet.<br />Navigate to a position and capture it.
        </div>
      )}
      ...
      <div className="space-y-1.5 max-h-80 overflow-y-auto pr-0.5">
        {keyframes.map((kf, i) => (
          <div
            key={kf.id}
            className="bg-gray-900 border border-gray-800 rounded-lg p-2 text-[11px] group"
          >
            {/* Header row */}
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              {editingId === kf.id ? (
                <input
                  ...
                  onBlur={() => {
                    updateKf(kf.id, { label: editLabel.trim() || kf.label });
                    setEditingId(null);
                  }}
                  onKeyDown={e => {
                    if (e.key === 'Enter') {
              ...
              <button
                onClick={() => onGoTo(kf)}
                title="Fly camera to this position"
                className="shrink-0 text-gray-700 hover:text-indigo-400 transition-colors text-[10px]"
              >
                ⟶
              </button>
              {/* Reorder */}
              <button
                onClick={() => move(kf.id, -1)}
                disabled={i === 0}
                className="shrink-0 text-gray-700 hover:text-gray-400 disabled:opacity-20 transition-colors"
              >
                ↑
              </button>
              <button
              ...
              {/* Delete */}
              <button
                onClick={() => setKeyframes(keyframes.filter(k => k.id !== kf.id))}
      ...
      {keyframes.length > 0 && (
        <div className="space-y-2 pt-1 border-t border-gray-800">
          <div className="text-[10px] text-gray-600 text-center">
            {keyframes.length} shot{keyframes.length > 1 ? 's' : ''} · {totalSec}s total
          </div>
          <div className="flex gap-2">
            {isPlaying ? (
              <button
                onClick={onStop}
                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-red-900/60 hover:bg-red-900 border border-red-800 text-red-200 text-xs font-medium transition-colors"
              >
                ⬛ Stop
              </button>
            ) : (
              <button
                onClick={onPlay}
                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-green-900/60 hover:bg-green-900 border border-green-800 text-green-200 text-xs font-medium transition-colors"
              >
                ▶ Play sequence
              </button>
            )}
            <button
              ...
              className="px-3 py-2 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-600 hover:text-gray-300 text-xs transition-colors"
            >
              🗑
            </button>
          </div>
